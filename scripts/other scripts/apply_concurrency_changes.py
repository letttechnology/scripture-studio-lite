...
# 1. Add class TranscriptionJob to the bottom of the file
job_class = """
...
if "class TranscriptionJob" not in code:
    code = code.replace("private fun formatDuration", job_class + "\nprivate fun formatDuration", 1)
    print("TranscriptionJob class added successfully!")
else:
    print("TranscriptionJob class already exists!")

# 2. Replace states at top of App
...
if orig_states in code:
    code = code.replace(orig_states, new_states, 1)
    print("App states replaced successfully!")
else:
    print("App states target NOT found!")

# 3. Replace Transcribe Button onClick handler and structure
import re

button_pattern = re.compile(r'// Transcribe Button\s*Button\(')
match_btn = button_pattern.search(code)
if match_btn:
    start_pos = match_btn.start()
    header_end = match_btn.end()
    
    # Trace braces of Button
    depth = 1
    pos = header_end
    n = len(code)
    ...
    if depth == 0:
        # replace from start_pos to pos + 1
        ...
        code = code[:start_pos] + new_button + code[pos+1:]
        print("Transcribe button replaced successfully!")
    else:
        print("Failed to find end of Transcribe Button brace!")
else:
    print("Transcribe button NOT found!")

# 4. Replace Transcript Text Pane Card
card_pattern = re.compile(r'// Transcript Text Pane Card\s*Card\(')
match_card = card_pattern.search(code)
if match_card:
    start_pos = match_card.start()
    header_end = match_card.end()
    
    # Trace braces of Card
    depth = 1
    pos = header_end
    n = len(code)
    while pos < n and depth > 0:
    ...
    if depth == 0:
                        ...
                        Column(modifier = Modifier.padding(16.dp)) {
                            ...
                            Row(
                                ...
                                Text(
                                    ...
                                    modifier = Modifier.weight(1f).padding(trailing = 8.dp)
                                )
        ...
        print("Transcript Card replaced successfully!")
    else:
        print("Failed to find end of Card brace!")
else:
    print("Transcript Card NOT found!")

# 5. Add Max Transcription Tabs option inside Settings Dialog
...
if settings_target in code:
    ...
    print("Max Tabs Limit added to Settings successfully!")
else:
    print("Settings target NOT found!")

# 6. Add Confirmation overlay for closing running transcription
...
if overlay_target in code:
    ...
    print("Confirm Close dialog overlay added successfully!")
else:
