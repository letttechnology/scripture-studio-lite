...
imports_addition = "import androidx.compose.foundation.horizontalScroll\nimport androidx.compose.foundation.rememberScrollState"
if "horizontalScroll" not in code:
    code = code.replace(
        "import androidx.compose.foundation.rememberScrollState",
        "import androidx.compose.foundation.rememberScrollState\n" + imports_addition,
        1
...
# 2. Add TranscriptionJob class at the bottom (before formatDuration)
job_class = """
...
# 5. Apply Transcribe Button replacement with robust nesting tracing and thread safety!
new_button = """Button(
                                    onClick = {
...
# Locate and replace Transcribe Button block
tr_idx = code.find("if (isTranscribing) return@Button")
if tr_idx != -1:
    start_pos = code.rfind("Button", 0, tr_idx)
    if start_pos != -1:
        p_depth = 0
        ...
        print("Could not find start_pos of Transcribe button!")
else:
    print("Could not find tr_idx in code!")

# 6. Replace Transcript Card
...
# 7. Add Max Transcription Tabs option inside Settings Dialog
...
# 8. Add Confirmation overlay for closing running transcription
...
if overlay_target in code:
    ...
    print("Confirmation overlay added!")

# Save back to App.kt
with open(path, "w", encoding="utf-8", newline="\r\n") as f:
    f.write(code)

print("Successfully applied workspace tabbed concurrency changes!")
